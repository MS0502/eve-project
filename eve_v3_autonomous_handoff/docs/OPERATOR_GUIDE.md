...
## Round101 operator guide — GitHub Release artifact path

Preferred release asset convention:
...
After downloading or otherwise obtaining the asset locally, audit it:

```bash
python -m adapters.medium_vector_restoration --candidate /path/to/vectors.npy
```

Install only if the audit reports:
...
shape: (30000, 300)
dtype: float32
target: seeds/subsets/cc.ko.300.subset.medium.30k/vectors.npy
```

The helper never downloads or installs the artifact automatically. Runtime mapping persistence remains disabled until post-install validation passes or a partial-validation path is explicitly approved.
