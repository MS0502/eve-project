...
def run_natural_language_v2_validation(verbose=True):
    def emit(message=""):
        if verbose:
            print(message)
    ...
    emit("=" * 70)
    emit("v2 NaturalLanguage 검증 ([11] echo + [12] sentiment)")
    emit("=" * 70)
    ...
    for text, expected in positive_tests:
        ...
        results.append(record(f"긍정 인식: '{text}'", ok, f"sent={u['sentiment']}"))
    ...
    for text, expected in negative_tests:
        u = nl.understand(text)
        ok = u['sentiment'] == expected
        results.append(record(f"부정 인식: '{text}'", ok, f"sent={u['sentiment']}"))
    ...
    emit(f"  cortisol↑: '{resp_a}'")
    emit(f"  oxytocin↑: '{resp_b}'")
    results.append(record("명령 응답 호르몬 따라 다름", resp_a != resp_b))
    ...
    emit(f"  dopamine↑: '{resp_c}'")
    emit(f"  baseline:  '{resp_d}'")
    results.append(record("긍정 감정 + dopamine↑ 차별화",
    ...
    for text in test_neg:
        ...
        results.append(record(f"'{text}' → 공감 응답", has_empathy, f"resp='{resp}'"))
    ...
    emit(f"v2 검증: {passed_count} / {total} 통과")
    emit("=" * 70)

    if passed_count == total:
        emit("\n🎉 v2 모든 검증 통과!")
        emit("→ [12] sentiment 풀 확장 (positive/negative 정확 분류)")
        emit("→ [11] respond echo 풀림 (입력 카테고리 그대로 반환 X)")
        emit("→ 호르몬/sentiment 다양화 작동")
        emit("→ 단순 호명 → 인사")
        emit("→ 부정 감정 → 공감")
        emit("→ 결정론 유지")
    else:
...
def test_natural_language_v2_validation_behavior():
    result = run_natural_language_v2_validation(verbose=True)
    assert result["passed"], f"v2 validation failed: {result['passed_count']} / {result['total']} passed"
