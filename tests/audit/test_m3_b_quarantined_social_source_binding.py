...
from scripts.audit.m3_b_quarantined_social_source_binding import audit_repository


def test_quarantined_social_source_binding_audit_is_deterministic_and_exact():
    first = audit_repository()
    second = audit_repository()

    assert first == second
    assert first["schema_version"] == "eve.m3-b.quarantined-social-source-binding-audit.v1"
    assert first["baseline_sha"] == "d82652d74b9051d227ec415b648ec209b9de8b3d"
    assert first["authority"] == "shadow_only_quarantined_social_source_binding"
    assert first["audit_fixture_only"] is True
    assert first["audit_fixture_is_production_observation"] is False
    assert first["appraised_binding_count"] == 7
    assert first["total_bound_axis_count"] == 19
    assert first["remaining_axis_count"] == 18
