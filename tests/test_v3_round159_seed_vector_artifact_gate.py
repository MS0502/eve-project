...
from adapters.external_seed_manifest import FASTTEXT_KOREAN_SUBSET_MEDIUM_30K_NAME, FASTTEXT_KOREAN_SUBSET_MEDIUM_30K_PATH


def _medium_vectors_present() -> bool:
    return (Path(FASTTEXT_KOREAN_SUBSET_MEDIUM_30K_PATH) / "vectors.npy").exists()


def _assert_medium_vectors_absent_or_ignored_untracked() -> None:
    path = Path(FASTTEXT_KOREAN_SUBSET_MEDIUM_30K_PATH) / "vectors.npy"
    if not path.exists():
        return
...
def test_round159_medium_artifact_gate_blocks_missing_real_vectors_without_dummy_data():
    ...
    _assert_medium_vectors_absent_or_ignored_untracked()
    if _medium_vectors_present():
        assert "operator_artifact_audit_failed" in gate["readiness_blockers"]
        assert gate["vector_contents_read"] is False
    else:
...
def test_round159_adapter_readiness_is_skip_free_and_does_not_load_adapter():
    ...
    _assert_medium_vectors_absent_or_ignored_untracked()
    assert gate["selected_subsets"][0]["vectors_file_present"] is _medium_vectors_present()
    assert gate["selected_subsets"][0]["vector_contents_read"] is False
    assert gate["selected_subsets"][0]["runtime_loaded"] is False
