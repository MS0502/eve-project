...
from core.m3_b_c2_fatigue_pressure_retention_activation import (
    C2FatiguePressureRetentionActivationError,
...
from core.m3_b_c2_reviewed_fatigue_pressure_integration import (
    ...
    C2ReviewedFatiguePressureIntegrationError,
...
def _fatigue_review() -> dict:
    return _review("docs/audit/M3_B_C2_REVIEWED_FATIGUE_PRESSURE_WITNESS.json")


def _seed_two(store: SQLiteShadowStore) -> None:
...
def test_reviewed_fatigue_public_review_matches_exact_pins() -> None:
    attestation, evidence = verify_public_review(_fatigue_review())
    assert attestation.attestation_digest == ATTESTATION_DIGEST
    assert evidence.evidence_digest == EVIDENCE_DIGEST
    assert _fatigue_review()["public_review_digest"] == PUBLIC_REVIEW_DIGEST
    assert evidence.axis == "fatigue_pressure"
    assert evidence.synthetic is False


def test_reviewed_fatigue_integration_advances_reviewed_not_retained_boundary() -> None:
    status = integration_status(_fatigue_review())
    capture = build_reviewed_capture(_fatigue_review())
    assert status.reviewed_real_operator_attestation_count == 3
    assert status.registered_runtime_provenance_verifier_count == 3
    assert status.registered_production_source_verifier_count == 3
    assert status.verified_positive_confidence_candidate_count == 3
    assert status.retained_real_observation_count == 2
...
def test_fatigue_sequence_three_append_requires_exact_prior_chain(tmp_path: Path) -> None:
    store = SQLiteShadowStore(tmp_path / "retained.sqlite3")
    store.initialize()
    _seed_two(store)

    receipt = append_reviewed_fatigue_pressure_observation(store, _fatigue_review())

    assert receipt.axis == "fatigue_pressure"
    assert receipt.sequence == 3
    assert receipt.store_ordinal == 3
    assert receipt.store_before_count == 2
    assert receipt.store_after_count == 3
    assert receipt.retained_real_observation_count_after_append == 3
...
def test_fatigue_sequence_three_cannot_replay_same_witness(tmp_path: Path) -> None:
    store = SQLiteShadowStore(tmp_path / "retained.sqlite3")
    store.initialize()
    _seed_two(store)
    append_reviewed_fatigue_pressure_observation(store, _fatigue_review())

    with pytest.raises(
        C2FatiguePressureRetentionActivationError,
        match="requires exactly two prior retained events",
    ):
        append_reviewed_fatigue_pressure_observation(store, _fatigue_review())


def test_reviewed_fatigue_public_review_fails_closed_on_tamper() -> None:
    review = _fatigue_review()
