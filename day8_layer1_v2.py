"""
Eve Day 8 - Layer 1 수정: 현실적 Poisson 배경
2000개 Poisson 뉴런 풀 → 각 exc 뉴런에 평균 50개 입력
"""
...
# ===== 개선된 Poisson 배경 =====
# 큰 입력 풀 → 각 뉴런 수많은 약한 입력
# 실제 피라미드 뉴런: 수천 개 시냅스 입력

# 흥분성 뉴런용: 2000개 Poisson 소스, 각 5 Hz
bg_input_exc = PoissonGroup(2000, rates=5*Hz)
bg_syn_exc = Synapses(bg_input_exc, exc_neurons, on_pre='v_post += 0.8')
bg_syn_exc.connect(p=0.05)  # 뉴런당 평균 100개 입력

# 억제성 뉴런용
bg_input_inh = PoissonGroup(500, rates=5*Hz)
bg_syn_inh = Synapses(bg_input_inh, inh_neurons, on_pre='v_post += 0.8')
bg_syn_inh.connect(p=0.1)  # 뉴런당 평균 50개 입력
...
print(f"Poisson 입력 풀: 2000 exc + 500 inh")
