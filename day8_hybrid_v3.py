...
Eve Day 8 하이브리드 v3
논문 기반 설계:
- Izhikevich 임계값 I_DC ≈ 3.78 (Hopf bifurcation)
- Subthreshold bias (I = 3 ± 0.5)
- Poisson conductance가 임계값 넘기게
- Inhibitory STDP (자동 균형)
- 6개 메커니즘 모두 유지

Reference:
- Izhikevich (2003) 표준 파라미터
- arxiv 1810.08488 (네트워크 동역학)
"""
...
# ===== 흥분성 뉴런 =====
# I를 subthreshold (3 ± 0.5) 로 설정
# Poisson이 합쳐서 임계 넘게 함
...
# 논문 기반: I = 3 ± 0.5 (subthreshold, 혼자선 발화 못함)
exc_neurons.I = '3.0 + rand() * 1.0'  
...
# FS 뉴런: 약간 더 낮은 subthreshold
inh_neurons.I = '2.5 + rand() * 1.0'
inh_neurons.g_ampa = 0
inh_neurons.g_gaba = 0


# ===== Poisson 배경 (적당한 강도) =====
# v1 (10Hz × 3.0 = 30)은 과활성
# v2 (5Hz × 1.5 = 7.5)는 약함
# v3: 15Hz × 2.0 = 30, 근데 I를 낮췄으니 적절
bg_input_exc = PoissonGroup(400, rates=15*Hz)
bg_syn_exc = Synapses(bg_input_exc, exc_neurons, on_pre='g_ampa_post += 2.0')
bg_syn_exc.connect(j='i')

bg_input_inh = PoissonGroup(100, rates=10*Hz)
bg_syn_inh = Synapses(bg_input_inh, inh_neurons, on_pre='g_ampa_post += 2.0')
...
print(f"\n=== Eve 하이브리드 v3 (논문 기반) ===")
print(f"설계 근거:")
print(f"  - Izhikevich I_DC 임계값 3.78 (Hopf bifurcation)")
print(f"  - I 값: 3.0~4.0 (subthreshold)")
print(f"  - Poisson: 15Hz × 2.0 (임계 넘기는 자극)")
print(f"\n시뮬 시간: 15초")
