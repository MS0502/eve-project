"""
Eve Day 8 v4: 대량 분산 입력
실제 피질 뉴런처럼 수천 개 약한 입력의 합산

Reference:
- Potjans & Diesmann 2014: 각 피라미드 뉴런 ~10,000 시냅스
...
exc_neurons.I = 0  # 외부 bias 없음 (Poisson 풀에만 의존)
...
# ===== 대량 분산 Poisson (실제 피질처럼) =====
# 2000개 입력 풀 → 각 뉴런에 약 300개 (p=0.15)
# 각 입력 5Hz × 0.3 strength
# 뉴런당 총 자극: 300 × 5Hz × 0.3 = 450 (분산 있음)

bg_input_exc = PoissonGroup(2000, rates=5*Hz)
bg_syn_exc = Synapses(bg_input_exc, exc_neurons, on_pre='g_ampa_post += 0.3')
bg_syn_exc.connect(p=0.15)

# 억제 뉴런: 조금 더 약한 입력
bg_input_inh = PoissonGroup(500, rates=5*Hz)
bg_syn_inh = Synapses(bg_input_inh, inh_neurons, on_pre='g_ampa_post += 0.3')
bg_syn_inh.connect(p=0.15)
...
print(f"\n=== Eve v4: 대량 분산 입력 ===")
print(f"Poisson 풀: 2000 exc + 500 inh")
print(f"뉴런당 입력: ~300개 (실제 피질 ~10000의 3%)")
print(f"각 입력 강도: 0.3 (매우 약함)")
print(f"총 자극: 300 × 5Hz × 0.3 = 450/초 per neuron")
print(f"bg→exc 시냅스: {len(bg_syn_exc)}")
print(f"bg→inh 시냅스: {len(bg_syn_inh)}")
print(f"\n시뮬 시간: 15초")
...
exc_hz = spikes_exc.num_spikes / (400 * 15)
inh_hz = spikes_inh.num_spikes / (100 * 15)
...
print(f"\n발화율 분산 (중요!):")
print(f"  최소: {final_rates.min():.2f} Hz")
print(f"  최대: {final_rates.max():.2f} Hz")
print(f"  표준편차: {final_rates.std():.2f} Hz")
