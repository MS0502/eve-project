"""Detached quarantined source bindings for the six risk-defense registry axes.

This module accepts caller-supplied immutable records that already passed an
explicit quarantine step and a verified bounded risk appraisal. It derives
positive-confidence registry evidence deterministically. It performs no source
acquisition, raw social-feedback ingestion, hardware polling, runtime hook,
scheduling, persistence, event append, owner mutation, observation-window
transition, or authority promotion.
"""
...
RAW_SCHEMA_VERSION = "eve.m3-b.quarantined-risk-raw-record.v1"
QUARANTINE_SCHEMA_VERSION = "eve.m3-b.risk-quarantine-trace.v1"
APPRAISAL_SCHEMA_VERSION = "eve.m3-b.risk-appraisal-trace.v1"
BINDING_SCHEMA_VERSION = "eve.m3-b.quarantined-risk-source-binding.v1"
BINDING_SET_SCHEMA_VERSION = "eve.m3-b.quarantined-risk-source-binding-set.v1"
SOURCE_FAMILY = "quarantined_risk_appraisal_trace"
ACQUISITION_METHOD = "explicit_caller_supplied_immutable_quarantined_risk_record"
VERIFICATION_METHOD = "exact_quarantine_appraisal_schema_range_identity_and_digest_verification"
QUARANTINE_METHOD = "deterministic_input_quarantine_before_risk_appraisal"
QUARANTINE_OUTCOME = "accepted_for_bounded_risk_appraisal"
APPRAISAL_METHOD = "deterministic_bounded_risk_defense_appraisal"
APPRAISAL_OUTCOME = "accepted_bounded_risk_defense_appraisal"
QUARANTINE_STATUS = "verified_input_quarantined_before_appraisal"
...
TOTAL_BOUND_AXIS_COUNT = 12
REMAINING_BINDING_BLOCKER = "REGISTRY_APPRAISED_25_AXIS_SOURCE_BINDINGS_INCOMPLETE"
...
class QuarantinedRiskSourceBindingError(ValueError):
    """Raised when a quarantined risk trace or binding fails closed."""


def _identifier(value: str, field: str, *, limit: int = 256) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > limit:
        raise QuarantinedRiskSourceBindingError(
...
def _unit(value: Any, field: str) -> float:
    if isinstance(value, bool) or not isinstance(value, Real):
        raise QuarantinedRiskSourceBindingError(f"{field} must be numeric")
    result = float(value)
...
def _manifest_entry(axis: str) -> RegistryObservationSourceEntry:
    for entry in registry_observation_source_manifest().entries:
        if entry.axis == axis:
            return entry
    raise QuarantinedRiskSourceBindingError(
        "risk-defense axis missing from source manifest"
    )
...
def quarantined_risk_raw_observation_digest(
    *,
    axis: str,
    logical_tick: int,
    observation_id: str,
    ...
    """Return the canonical digest for one quarantined risk record."""

    if axis not in RISK_DEFENSE_AXES:
        raise QuarantinedRiskSourceBindingError("unsupported risk-defense axis")
    ...
    if appraisal_input_digest != quarantine_integrity_digest:
        raise QuarantinedRiskSourceBindingError(
            "appraisal input must be the exact verified quarantine output"
        )
    ...
    return _digest(
        ...
        "quarantined_risk_raw_observation",
    )
...
def _validate_raw_values(axis: str, raw: Mapping[str, Any]) -> None:
    if "appraisal_version" in raw and raw["appraisal_version"] != APPRAISAL_SCHEMA_VERSION:
        raise QuarantinedRiskSourceBindingError(
            "appraisal_version must match the canonical risk appraisal schema"
        )
    if axis == "threat_pressure":
        _unit(raw["impact_score"], "impact_score")
        _unit(raw["source_trust"], "source_trust")
        _unit(raw["threat_probability"], "threat_probability")
    ...
    if axis == "uncertainty_pressure":
        _nonnegative_int(raw["conflict_count"], "conflict_count")
        _unit(raw["missing_evidence_ratio"], "missing_evidence_ratio")
        _unit(raw["source_reliability"], "source_reliability")
        _unit(raw["verification_gap"], "verification_gap")
        return
    ...
    if axis == "boundary_defense":
        _nonnegative_int(raw["boundary_violation_count"], "boundary_violation_count")
        for field in ("intent_confidence", "persistence_score", "remedy_available"):
            _unit(raw[field], field)
        return
    if axis == "trust_risk":
        _nonnegative_int(raw["contradiction_count"], "contradiction_count")
        for field in ("reversibility", "source_reliability", "verification_depth"):
            _unit(raw[field], field)
        return
    if axis == "exposure_risk":
        _unit(raw["audience_scope"], "audience_scope")
        _enum(raw["authorization_status"], "authorization_status", _AUTHORIZATION_RISK)
        _unit(raw["persistence_risk"], "persistence_risk")
        _unit(raw["reversibility"], "reversibility")
        _enum(raw["sensitivity_class"], "sensitivity_class", _SENSITIVITY_RISK)
        return
    raise QuarantinedRiskSourceBindingError("unsupported risk-defense axis")


def _record_score(record: "QuarantinedRiskRawRecord") -> float:
    raw = record.raw_mapping
    if record.axis == "threat_pressure":
        values = (
            raw["impact_score"],
            1.0 - raw["source_trust"],
            raw["threat_probability"],
        )
    elif record.axis == "uncertainty_pressure":
        values = (
            _count_pressure(raw["conflict_count"], "conflict_count"),
            raw["missing_evidence_ratio"],
            1.0 - raw["source_reliability"],
            raw["verification_gap"],
        )
    elif record.axis == "self_protection":
        values = (
            raw["capability_limit"],
            raw["exposure_scope"],
            1.0 - raw["reversibility"],
            raw["threat_pressure_input"],
        )
    elif record.axis == "boundary_defense":
        values = (
            _count_pressure(raw["boundary_violation_count"], "boundary_violation_count"),
            raw["intent_confidence"],
            raw["persistence_score"],
            1.0 - raw["remedy_available"],
        )
    elif record.axis == "trust_risk":
        values = (
            _count_pressure(raw["contradiction_count"], "contradiction_count"),
            1.0 - raw["reversibility"],
            1.0 - raw["source_reliability"],
            1.0 - raw["verification_depth"],
        )
    elif record.axis == "exposure_risk":
        values = (
            raw["audience_scope"],
            _AUTHORIZATION_RISK[raw["authorization_status"]],
            raw["persistence_risk"],
            1.0 - raw["reversibility"],
            _SENSITIVITY_RISK[raw["sensitivity_class"]],
        )
    else:
        raise QuarantinedRiskSourceBindingError("unsupported risk-defense axis")
...
class QuarantinedRiskRawRecord:
    axis: str
    logical_tick: int
    observation_id: str
    source_instance_id: str
    ...
    def __post_init__(self) -> None:
        if self.axis not in RISK_DEFENSE_AXES:
            raise QuarantinedRiskSourceBindingError("unsupported risk-defense axis")
        ...
        for field, expected in expected_provenance.items():
            if getattr(self, field) != expected:
                raise QuarantinedRiskSourceBindingError(
                    f"raw record {field} does not match the canonical quarantined-risk provenance contract"
                )
        if self.quarantine_verified is not True or self.appraisal_verified is not True:
            raise QuarantinedRiskSourceBindingError(
                "risk-defense evidence requires exact quarantine and appraisal verification"
            )
        ...
        if any(
            ...
            raise QuarantinedRiskSourceBindingError(
                "risk-defense evidence cannot use raw social feedback, direct hardware, synthetic, proposal-only, circular, or runtime-polled input"
            )
        ...
        expected_digest = quarantined_risk_raw_observation_digest(
            axis=self.axis,
        ...
        if self.schema_version != RAW_SCHEMA_VERSION:
            raise QuarantinedRiskSourceBindingError(
                "unsupported quarantined-risk raw schema"
            )
    ...
    def recalculated_raw_observation_digest(self) -> str:
        return quarantined_risk_raw_observation_digest(
            axis=self.axis,
            logical_tick=self.logical_tick,
            observation_id=self.observation_id,
            source_instance_id=self.source_instance_id,
...
@dataclass(frozen=True, slots=True)
class QuarantinedRiskSourceBinding:
    axis: str
    ...
    def __post_init__(self) -> None:
        if self.axis not in RISK_DEFENSE_AXES:
            raise QuarantinedRiskSourceBindingError("unsupported binding axis")
        ...
        if self.schema_version != BINDING_SCHEMA_VERSION or self.authority != SHADOW_AUTHORITY:
            raise QuarantinedRiskSourceBindingError(
                "quarantined risk binding must use the exact shadow-only schema"
            )
    ...
    def binding_digest(self) -> str:
        return _digest(self.to_mapping(), "quarantined_risk_source_binding")


@dataclass(frozen=True, slots=True)
class QuarantinedRiskSourceBindingSet:
    bindings: tuple[QuarantinedRiskSourceBinding, ...]
    schema_version: str = BINDING_SET_SCHEMA_VERSION
    authority: str = SHADOW_AUTHORITY
    total_bound_axis_count: int = TOTAL_BOUND_AXIS_COUNT
    remaining_axis_count: int = 25
    production_capture_present: bool = False
    observation_window_started: bool = False
    m3_b_complete: bool = False
    m3_c_open: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if any(type(item) is not QuarantinedRiskSourceBinding for item in bindings):
            raise QuarantinedRiskSourceBindingError(
                "binding set requires exact immutable binding types"
            )
        if len(bindings) != 6 or tuple(item.axis for item in bindings) != RISK_DEFENSE_AXES:
            raise QuarantinedRiskSourceBindingError(
                "quarantined risk binding set must preserve exact six-axis order"
            )
        if (
            ...
            or self.remaining_axis_count != 25
        ):
            raise QuarantinedRiskSourceBindingError(
                "unsupported quarantined-risk binding-set contract"
            )
        if any(
            (
                self.production_capture_present,
    ...
    def binding_set_digest(self) -> str:
        return _digest(self.to_mapping(), "quarantined_risk_source_binding_set")


def quarantined_risk_source_bindings() -> QuarantinedRiskSourceBindingSet:
    bindings = []
    for axis in RISK_DEFENSE_AXES:
        entry = _manifest_entry(axis)
        bindings.append(
            QuarantinedRiskSourceBinding(
                axis=axis,
                source_contract_id=entry.source_contract_id,
                binding_id=f"eve:m3-b:quarantined-risk-binding:{axis}:v1",
                raw_schema_version=RAW_SCHEMA_VERSION,
                quarantine_schema_version=QUARANTINE_SCHEMA_VERSION,
                appraisal_schema_version=APPRAISAL_SCHEMA_VERSION,
                required_raw_fields=entry.required_raw_fields,
    ...
    return QuarantinedRiskSourceBindingSet(bindings=tuple(bindings))


def derive_quarantined_risk_axis_evidence(
    records: Sequence[QuarantinedRiskRawRecord],
) -> RegistryAxisPositiveConfidenceEvidence:
    """Derive one detached evidence record from quarantined risk appraisals."""

    items = tuple(records)
    if not items or any(type(item) is not QuarantinedRiskRawRecord for item in items):
        raise QuarantinedRiskSourceBindingError(
            "records must contain exact immutable quarantined-risk raw records"
        )
    axis = items[0].axis
    if any(item.axis != axis for item in items):
        raise QuarantinedRiskSourceBindingError("records cannot mix axes")
    ticks = tuple(item.logical_tick for item in items)
    if ticks != tuple(sorted(ticks)):
        raise QuarantinedRiskSourceBindingError("record ticks must be sorted")
    if len(set(ticks)) != len(ticks):
        raise QuarantinedRiskSourceBindingError("record ticks must be unique")
    for field in (
        "observation_id",
        "source_snapshot_id",
        "quarantine_trace_id",
    ...
    binding = next(
        item
        for item in quarantined_risk_source_bindings().bindings
        if item.axis == axis
    )
    span = items[-1].logical_tick - items[0].logical_tick
    if len(items) < binding.minimum_raw_record_count:
        raise QuarantinedRiskSourceBindingError("insufficient raw record count")
    ...
    source_integrity_digest = _digest(
        ...
        "quarantined_risk_source_integrity",
    )
    return RegistryAxisPositiveConfidenceEvidence(
        ...
        observation_id=f"quarantined-risk:{axis}:{raw_bundle_digest[:24]}",
        source_family=SOURCE_FAMILY,
        source_instance_id=items[0].source_instance_id,
        source_snapshot_id=(
            f"quarantined-risk:{axis}:{items[0].logical_tick}:"
