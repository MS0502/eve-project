"""Sequence-four durable retention activation for reviewed ``recovery_need``.

The append is permitted only when the operator-private retention stream already
contains the exact immutable sequence-1 prediction-error, sequence-2 energy-budget,
and sequence-3 fatigue-pressure events pinned by repository receipts. Importing
...
RETENTION_SCHEMA_VERSION = "eve.m3-b.c2-recovery-need-retention-activation.v1"
RECEIPT_SCHEMA_VERSION = "eve.m3-b.c2-recovery-need-retention-receipt.v1"
RETENTION_PRODUCER_VERSION = RETENTION_SCHEMA_VERSION
EVENT_ID = "m3b:c2:retained:recovery_need:000004"
SEQUENCE = 4
CORRELATION_ID = "m3b:c2:reviewed-recovery-need-witness:f0edb0520167"
...
PRIOR_EVENT_ENVELOPE_DIGEST = "f81d43bf40b4dc76130767f91b65ad2503bc70e61ef718fe3d0e446528d1a7e3"
PRIOR_STORE_CHAIN_DIGEST = "b73ec7ea2f5e6e4e8eda5b57b4f6464a17d94e56026718b5b2e15cbca9f2162f"
PRIOR_AXIS = "fatigue_pressure"
PRIOR_PUBLIC_REVIEW_DIGEST = "4b88c7734234ac2982836b95bf392fe143bc928119d4af515e576b39e480af61"
_RECEIPT_TOKEN = object()


class C2RecoveryNeedRetentionActivationError(ValueError):
    pass
...
def _canonical(value: Mapping[str, Any], field: str) -> str:
    try:
        ...
        raise C2RecoveryNeedRetentionActivationError(
...
def _payload(capture: C2ReviewedRecoveryNeedCapture) -> dict[str, Any]:
    return {
...
def build_retention_event(capture: C2ReviewedRecoveryNeedCapture) -> EventEnvelope:
    if type(capture) is not C2ReviewedRecoveryNeedCapture:
        raise C2RecoveryNeedRetentionActivationError(
            "retention requires exact reviewed recovery-need capture"
        )
    if (
        ...
        raise C2RecoveryNeedRetentionActivationError(
            "reviewed recovery-need capture is not retention eligible"
        )
...
def _require_exact_prior_retention(
    ...
    if len(existing) != 3:
        raise C2RecoveryNeedRetentionActivationError(
            "recovery-need retention requires exactly three prior retained events"
        )
    first, second, prior = existing
    if (
        ...
        raise C2RecoveryNeedRetentionActivationError(
            "existing retention history does not match pinned sequence-one/two/three receipts"
        )
    return first, second, prior


@dataclass(frozen=True, slots=True)
class C2RecoveryNeedRetentionReceipt:
    axis: str
    ...
    retained_real_observation_count_after_append: int = 4
    ...
    def __post_init__(self, _issuance_token: object | None) -> None:
        if _issuance_token is not _RECEIPT_TOKEN:
            raise C2RecoveryNeedRetentionActivationError(
                "recovery-need receipt must be issued by durable append"
            )
        if (
            self.axis != AXIS
            ...
            or self.store_ordinal != 4
            or self.store_before_count != 3
            or self.store_after_count != 4
            ...
            or self.retained_real_observation_count_after_append != 4
            or self.authority != SHADOW_AUTHORITY
            or self.schema_version != RECEIPT_SCHEMA_VERSION
        ):
            raise C2RecoveryNeedRetentionActivationError(
                "recovery-need receipt does not prove exact sequence-four append"
            )
        if any(
            (
            ...
            raise C2RecoveryNeedRetentionActivationError(
                "recovery-need retention receipt cannot open later authority"
            )
    ...
    def receipt_digest(self) -> str:
        return _digest(self.to_mapping(), "c2_recovery_need_retention_receipt")


def append_reviewed_recovery_need_observation(
    store: SQLiteShadowStore,
    public_review: Mapping[str, Any],
) -> C2RecoveryNeedRetentionReceipt:
    if type(store) is not SQLiteShadowStore:
        raise C2RecoveryNeedRetentionActivationError(
            "recovery-need retention requires exact SQLiteShadowStore"
        )
    ...
    if (
        ...
        or receipt.ordinal != 4
        or receipt.before_count != 3
        or receipt.after_count != 4
        ...
        raise C2RecoveryNeedRetentionActivationError(
            "underlying SQLite receipt does not prove exact recovery-need append"
        )
    ...
    if (
        ...
        or readback[2].digest != PRIOR_EVENT_ENVELOPE_DIGEST
        or readback[3] != event
    ):
        raise C2RecoveryNeedRetentionActivationError(
            "sequence-four retained readback does not match exact event history"
        )
    return C2RecoveryNeedRetentionReceipt(
        axis=AXIS,
        public_review_digest=PUBLIC_REVIEW_DIGEST,
        attestation_digest=ATTESTATION_DIGEST,
        evidence_digest=EVIDENCE_DIGEST,
