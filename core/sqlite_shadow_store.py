...
CREATE TABLE IF NOT EXISTS migrations(
 ordinal INTEGER PRIMARY KEY,migration_id TEXT UNIQUE NOT NULL,
...
CREATE TABLE IF NOT EXISTS events(
 ...
 previous_chain_digest TEXT NOT NULL,chain_digest TEXT NOT NULL,
 UNIQUE(stream_id,sequence));
CREATE TABLE IF NOT EXISTS snapshots(
 ordinal INTEGER PRIMARY KEY AUTOINCREMENT,snapshot_id TEXT UNIQUE NOT NULL,
 stream_id TEXT NOT NULL,through_sequence INTEGER NOT NULL,
 through_event_id TEXT,through_event_digest TEXT,state_schema_version TEXT NOT NULL,
 state_json TEXT NOT NULL,state_digest TEXT NOT NULL,manifest_json TEXT NOT NULL,
 manifest_digest TEXT NOT NULL,snapshot_digest TEXT NOT NULL);
"""
_TRIGGERS = """
CREATE TRIGGER IF NOT EXISTS metadata_no_update BEFORE UPDATE ON metadata BEGIN SELECT RAISE(ABORT,'append-only metadata'); END;
CREATE TRIGGER IF NOT EXISTS metadata_no_delete BEFORE DELETE ON metadata BEGIN SELECT RAISE(ABORT,'append-only metadata'); END;
CREATE TRIGGER IF NOT EXISTS migrations_no_update BEFORE UPDATE ON migrations BEGIN SELECT RAISE(ABORT,'append-only migrations'); END;
CREATE TRIGGER IF NOT EXISTS migrations_no_delete BEFORE DELETE ON migrations BEGIN SELECT RAISE(ABORT,'append-only migrations'); END;
...
CREATE TRIGGER IF NOT EXISTS snapshots_no_update BEFORE UPDATE ON snapshots BEGIN SELECT RAISE(ABORT,'append-only snapshots'); END;
CREATE TRIGGER IF NOT EXISTS snapshots_no_delete BEFORE DELETE ON snapshots BEGIN SELECT RAISE(ABORT,'append-only snapshots'); END;
"""
_MIGRATION = _canon(
    {"migration_id": "m2-a.initial.v1", "from_version": None, "to_version": STORE_SCHEMA_VERSION,
     "tables": ["events", "metadata", "migrations", "snapshots"], "append_only": True},
    "migration_manifest",
)
...
class SQLiteShadowStore:
    ...
    @contextmanager
    def _connect(self, *, require_initialized: bool = True) -> Iterator[sqlite3.Connection]:
    ...
    def _validate_schema(connection: sqlite3.Connection) -> None:
        expected = {"events", "metadata", "migrations", "snapshots"}
        actual = {str(row[0]) for row in connection.execute("SELECT name FROM sqlite_master WHERE type='table'")}
        if not expected <= actual:
            raise SchemaMismatch("required M2-A tables are missing")
        ...
        if len(rows) != 1 or tuple(rows[0]) != (1, "m2-a.initial.v1", MIGRATION_SCHEMA_VERSION, _MIGRATION, _MIGRATION_DIGEST):
            raise SchemaMismatch("migration history differs from M2-A v1")

    def _counts(self, connection: sqlite3.Connection) -> tuple[int, int, str]:
        count, size = connection.execute("SELECT COUNT(*),COALESCE(SUM(event_bytes),0) FROM events").fetchone()
        row = connection.execute("SELECT chain_digest FROM events ORDER BY ordinal DESC LIMIT 1").fetchone()
        return int(count), int(size), GENESIS_DIGEST if row is None else str(row[0])
    ...
    def append_many(self, envelopes: Iterable[EventEnvelope]) -> tuple[AppendReceipt, ...]:
        ...
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            try:
                before_count, before_bytes, chain = self._counts(connection)
                known = {str(row[0]) for row in connection.execute("SELECT event_id FROM events")}
                last_seq = {str(row[0]): int(row[1]) for row in connection.execute("SELECT stream_id,MAX(sequence) FROM events GROUP BY stream_id")}
                ...
                for envelope, text, event_bytes in materials:
                    ...
                    connection.execute(
                        ...
                        (envelope.event_id, envelope.stream_id, envelope.sequence, text, envelope.digest, event_bytes, chain, next_chain),
                    )
                    row = connection.execute("SELECT * FROM events WHERE event_id=?", (envelope.event_id,)).fetchone()
                    if row is None or _event_from_json(str(row["event_json"]), str(row["envelope_digest"])) != envelope:
                        raise PersistedEventCorruption("event readback failed before commit")
                    transition = _digest(
                        {"after_chain_digest": next_chain, "after_count": count + 1, "before_chain_digest": chain,
                         "before_count": count, "event_id": envelope.event_id},
                        "append_transition",
                    )
                    receipts.append(AppendReceipt(ordinal, envelope.event_id, envelope.stream_id, envelope.sequence,
                                                   envelope.digest, count, count + 1, chain, next_chain,
        ...
        with self._connect() as connection:
            if stream_id is None:
                ...
                rows = connection.execute(
                    "SELECT event_json,envelope_digest FROM events WHERE stream_id=? AND sequence>? ORDER BY sequence",
                    (stream_id, after_sequence),
                ).fetchall()
            return tuple(_event_from_json(str(row[0]), str(row[1])) for row in rows)

    def snapshot_due(self, stream_id: str) -> bool:
        with self._connect() as connection:
            head = connection.execute("SELECT COALESCE(MAX(sequence),0) FROM events WHERE stream_id=?", (stream_id,)).fetchone()[0]
            snap = connection.execute("SELECT COALESCE(MAX(through_sequence),0) FROM snapshots WHERE stream_id=?", (stream_id,)).fetchone()[0]
    ...
    def write_snapshot(self, *, snapshot_id: str, stream_id: str, through_sequence: int,
        ...
        manifest_json = _canon(
            {"canonical_bytes": len(state_json.encode("utf-8")), "hash_algorithm": "sha256",
             "serialization_schema": STATE_ENCODING_VERSION, "state_schema_version": state_schema_version,
             "top_level_key_count": len(state), "top_level_keys": sorted(state)},
            "snapshot_manifest",
        )
        ...
        with self._connect() as connection:
            ...
            try:
                ...
                head = connection.execute(
                    "SELECT event_id,envelope_digest,sequence FROM events WHERE stream_id=? ORDER BY sequence DESC LIMIT 1",
                    (stream_id,),
                ).fetchone()
                if head is None:
                    ...
                    if through_sequence != int(head["sequence"]):
                        raise SnapshotCorruption("snapshot must bind the current stream head")
                    event_id, event_digest = str(head["event_id"]), str(head["envelope_digest"])
                snapshot_digest = _digest(
                    {"authority": SHADOW_AUTHORITY, "manifest_digest": manifest_digest, "schema_version": SNAPSHOT_SCHEMA_VERSION,
                     "snapshot_id": snapshot_id, "state_digest": state_digest, "state_schema_version": state_schema_version,
                     "stream_id": stream_id, "through_event_digest": event_digest, "through_event_id": event_id,
                     "through_sequence": through_sequence},
                    "snapshot_material",
                )
                connection.execute(
                    "INSERT INTO snapshots(snapshot_id,stream_id,through_sequence,through_event_id,through_event_digest,state_schema_version,state_json,state_digest,manifest_json,manifest_digest,snapshot_digest) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                    (snapshot_id, stream_id, through_sequence, event_id, event_digest, state_schema_version,
                     state_json, state_digest, manifest_json, manifest_digest, snapshot_digest),
                )
                ...
                transition = _digest({"snapshot_digest": persisted.snapshot_digest, "snapshot_id": snapshot_id}, "snapshot_transition")
                connection.commit()
                return SnapshotReceipt(snapshot_id, stream_id, through_sequence, state_digest, manifest_digest,
                                       snapshot_digest, transition, True)
            except (sqlite3.DatabaseError, ShadowPersistenceError):
                connection.rollback()
                raise
    ...
    def latest_valid_snapshot(self, stream_id: str) -> SnapshotSelection:
        with self._connect() as connection:
            rows = connection.execute("SELECT * FROM snapshots WHERE stream_id=? ORDER BY ordinal DESC", (stream_id,)).fetchall()
            rejected: list[str] = []
            selected = None
            for row in rows:
                try:
            ...
            material = {"rejected_snapshot_ids": rejected, "selected_snapshot_id": None if selected is None else selected.snapshot_id}
            return SnapshotSelection(selected, tuple(rejected), bool(rejected and selected is not None), _digest(material, "snapshot_selection"))
    ...
    def _snapshot_from_row(self, connection: sqlite3.Connection, row: sqlite3.Row | None) -> Snapshot:
        ...
        try:
            snap = Snapshot(int(row["ordinal"]), str(row["snapshot_id"]), str(row["stream_id"]), int(row["through_sequence"]),
                            ...
                            str(row["state_schema_version"]), str(row["state_json"]), str(row["state_digest"]),
                            str(row["manifest_json"]), str(row["manifest_digest"]), str(row["snapshot_digest"]))
        ...
        expected_manifest = {"canonical_bytes": len(snap.state_json.encode("utf-8")), "hash_algorithm": "sha256",
                             "serialization_schema": STATE_ENCODING_VERSION, "state_schema_version": snap.state_schema_version,
                             "top_level_key_count": len(state), "top_level_keys": sorted(state)}
        if _canon(expected_manifest, "snapshot_manifest") != snap.manifest_json or _sha(snap.manifest_json) != snap.manifest_digest:
        ...
        if snap.through_sequence == 0:
            ...
            event = connection.execute("SELECT event_id,envelope_digest FROM events WHERE stream_id=? AND sequence=?",
                                       (snap.stream_id, snap.through_sequence)).fetchone()
            if event is None or (snap.through_event_id, snap.through_event_digest) != (str(event[0]), str(event[1])):
                raise SnapshotCorruption("snapshot boundary event mismatch")
        expected_digest = _digest(
            {"authority": SHADOW_AUTHORITY, "manifest_digest": snap.manifest_digest, "schema_version": SNAPSHOT_SCHEMA_VERSION,
             "snapshot_id": snap.snapshot_id, "state_digest": snap.state_digest, "state_schema_version": snap.state_schema_version,
             "stream_id": snap.stream_id, "through_event_digest": snap.through_event_digest,
             "through_event_id": snap.through_event_id, "through_sequence": snap.through_sequence},
            "snapshot_material",
        )
    ...
    def restore_verified(self, *, stream_id: str, initial_state: StateT,
                         reducer: Callable[[StateT, EventEnvelope], StateT],
                         state_to_mapping: Callable[[StateT], Mapping[str, Any]],
                         state_from_mapping: Callable[[Mapping[str, Any]], StateT]) -> RestoreReport[StateT]:
        ...
        transition = _digest({"final_state_digest": left_digest, "replayed_event_count": len(events),
                              "selection_digest": selection.selection_digest, "stream_id": stream_id}, "restore_transition")
        return RestoreReport(left, left_digest, right_digest, len(events), snapshot_id,
                             selection.rejected_snapshot_ids, transition, True)
    ...
    def integrity_check(self) -> IntegrityReport:
            ...
            with self._connect() as connection:
                ...
                for expected_ordinal, row in enumerate(rows, 1):
                    try:
                        ...
                        expected_chain = _digest({"envelope_digest": envelope.digest, "ordinal": expected_ordinal,
                                                  "previous_chain_digest": chain}, "event_chain")
                        ...
                        errors.append(f"event:{expected_ordinal}:{exc}")
                        break
                if event_count > self._policy.max_event_count or total_bytes > self._policy.max_event_bytes:
                ...
                if snapshot_count > self._policy.max_snapshot_count:
        ...
        material = {"chain_head_digest": chain, "errors": errors, "event_count": event_count,
                    "schema_version": INTEGRITY_SCHEMA_VERSION, "snapshot_count": snapshot_count}
    ...
    def create_backup(self, backup_directory: str | Path, *, backup_ordinal: int) -> BackupReceipt:
        ...
        try:
            if str(check.execute("PRAGMA integrity_check").fetchone()[0]) != "ok":
                raise BackupPolicyError("backup integrity check failed")
        finally:
            check.close()
