...
def _normalize_items(items, source, situation_id):
    ...
    source_map={i["inference_item_id"]: i for i in source["inference_items"]}
    ...
    for item in sorted(items, key=_canonical_sort_key):
        ...
        if item["source_inference_item_id"] not in source_map: return None, "source_inference_item_not_in_context"
        src = source_map[item["source_inference_item_id"]]
        if item["source_conclusion_ref_id"] != src["conclusion_ref_id"]: return None, "source_conclusion_ref_not_in_source_item"
        expected_kind = INFERENCE_TO_CONCLUSION_KIND[src["inference_kind"]]
        if item["conclusion_kind"] != expected_kind or item["candidate_role"] != src["candidate_role"] or item["subject_ref_id"] != src["subject_ref_id"]: return None, "source_projection_mismatch"
        if ("object_ref_id" in item) != ("object_ref_id" in src): return None, "source_projection_mismatch"
        if "object_ref_id" in item and item["object_ref_id"] != src["object_ref_id"]: return None, "source_projection_mismatch"
        for numeric in ("confidence_candidate", "coherence_candidate"):
            if (numeric in item) != (numeric in src): return None, "source_projection_mismatch"
            if numeric in item and (type(item[numeric]) is not type(src[numeric]) or item[numeric] != src[numeric]): return None, "source_projection_mismatch"
        if item["situation_id"] != situation_id or item["situation_id"] != src["situation_id"]: return None, "conclusion_item_situation_mismatch"
        ...
        if item["premise_ref_ids"] != sorted(src["premise_ref_ids"]) or item["hypothesis_context_ref_ids"] != sorted(src["hypothesis_context_ref_ids"]) or item.get("evidence_ref_ids", []) != sorted(src.get("evidence_ref_ids", [])): return None, "source_projection_mismatch"
...
def _canonical_id(conclusion_type, boundary, confidence, source_id, situation_id, items, metadata, source_context):
    raw={"schema_version":VERSION,"conclusion_type":conclusion_type,"conclusion_boundary_classification":boundary,"conclusion_confidence_state":confidence,"source_inference_context_id":source_id,"source_inference_context":source_context,"situation_id":situation_id,"conclusion_items":items,"metadata":metadata}
...
def validate_virtual_world_situation_conclusion_candidate(payload: Any) -> bool:
    try:
        ...
        source = payload.get("source_inference_context")
        if not validate_virtual_world_situation_inference_context(source): return False
        if source.get("inference_context_id") != payload.get("source_inference_context_id"): return False
        rebuilt = build_virtual_world_situation_conclusion_candidate(source, payload.get("conclusion_type"), payload.get("conclusion_items"), payload.get("metadata"))
