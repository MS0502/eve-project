"""
Eve Day 8 - Layer 1: Poisson 배경 노이즈
상수 입력 → 실제 뇌처럼 Poisson 랜덤 입력
"""
...
inh_neurons.I = 0  # 상수 입력 제거!


# ===== Layer 1: Poisson 배경 노이즈 =====
# 각 뉴런마다 독립적인 Poisson 스파이크 입력
# 실제 뇌의 감각/배경 활동 모방

# 흥분성 뉴런에 대한 배경 입력 (5 Hz, 실제 뇌 수준)
bg_input_exc = PoissonGroup(400, rates=5*Hz)
bg_syn_exc = Synapses(bg_input_exc, exc_neurons, on_pre='v_post += 3.0')
bg_syn_exc.connect(j='i')  # 1:1 연결

# 억제성 뉴런에 대한 배경 입력
bg_input_inh = PoissonGroup(100, rates=3*Hz)
bg_syn_inh = Synapses(bg_input_inh, inh_neurons, on_pre='v_post += 3.0')
