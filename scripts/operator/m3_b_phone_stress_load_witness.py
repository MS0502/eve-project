#!/usr/bin/env python3
"""Run three new real full-engine phone interactions and emit a stress-load witness.

The interaction texts, raw process CPU/wall observations, kernel load-average
observations, and private nonce remain operator-private. The public file contains
only bounded derived evidence, method identifiers, and cryptographic digests.

This command does not reuse or replay any previously retained witness interaction.
"""
...
from core.m3_b_phone_stress_load_witness import (  # noqa: E402
    DEFAULT_SOURCE_INSTANCE_ID,
    ENTRYPOINT_ID,
    PhoneStressLoadRuntimeSnapshot,
    build_phone_stress_load_witness,
)
from main import build_full_engine  # noqa: E402

PRIVATE_FILENAME = "stress_load_witness_private_v1.json"
PUBLIC_FILENAME = "stress_load_witness_public_review_v1.json"
...
def _capture_interaction(
    ...
    *,
    ...
    if cpu_delta < 0.0 or wall_delta <= 0.0:
        raise SystemExit("stress-load runtime counters did not advance monotonically")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        dest="inputs",
        required=True,
        help="New real operator interaction text. Supply exactly three times.",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if len(args.inputs) != 3:
        raise SystemExit("phone stress-load witness requires exactly three --input values")
    ...
    _atomic_json(private_root / PRIVATE_FILENAME, witness.private_mapping())
