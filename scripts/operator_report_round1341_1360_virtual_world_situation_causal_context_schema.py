...
def main():
    ...
    report = {
        ...
        "causal_chain_validation_passed": validate_virtual_world_situation_causal_context(chain) is True and bad_chain["blocked_reasons"] == ["causal_chain_requires_multiple_links"],
        ...
        "scheduling_alias_forbidden_passed": build_virtual_world_situation_causal_context("direct_cause_candidate", "s1", "s2", [LINK], {"nested":[{"timer_requested":True}]})["blocked_reasons"] == ["timer_requested"] and build_virtual_world_situation_causal_context("direct_cause_candidate", "s1", "s2", [{**LINK, "unknown":{"cron_expression":"* * * * *"}}])["blocked_reasons"] == ["malformed_forbidden_request_field"],
